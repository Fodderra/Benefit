...
function FlipbookViewer({ issue }: { issue: Issue }) {
    ...
    <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.08)', padding: '48px', borderRadius: '2px' }}>
          ...
          <div style={{ width: 'clamp(160px, 25vw, 320px)', height: 'clamp(220px, 35vw, 450px)', background: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: '2px solid #000', position: 'relative', overflow: 'hidden' }}>
            {page === 0 ? (
              <div style={{ width: '100%', height: '100%', background: '#111', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '24px' }}>
                {issue.cover && <img src={issue.cover} alt={issue.title} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.8 }} />}
              ...
              <div style={{ padding: '24px', width: '100%' }}>
                ...
                <p style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.45)', lineHeight: 1.7 }}>Premium business content curated for Georgia&apos;s most influential audience.</p>
              </div>
            )}
          </div>
          {/* Right page */}
          <div style={{ width: 'clamp(160px, 25vw, 320px)', height: 'clamp(220px, 35vw, 450px)', background: '#161616', overflow: 'hidden' }}>
            {page === 0 ? (
              <div style={{ padding: '24px', width: '100%' }}>
                ...
                {['The Power of Influence', 'Luxury Reimagined', 'Business Trends', 'Premium Lifestyle', 'Partner Spotlight', 'Event Recap'].map((item, i) => (
      ...
      {issue.pdfUrl && (
        <div style={{ textAlign: 'center', marginTop: '32px' }}>
          <a href={issue.pdfUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', border: '1px solid rgba(255,255,255,0.3)', color: '#fff', padding: '12px 28px', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', textDecoration: 'none', transition: 'border-color 0.2s' }}>
            Download PDF ↓
          </a>
        </div>
      )}
    ...
    <main style={{ background: '#000000', minHeight: '100vh' }}>
      ...
      <section style={{ paddingTop: '120px', paddingBottom: '80px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 48px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 7rem)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1, color: '#FFFFFF', textTransform: 'uppercase', marginBottom: '24px' }}>Magazine</h1>
          <p style={{ color: 'rgba(255,255,255,0.55)', maxWidth: '520px', lineHeight: 1.7, fontSize: '1rem' }}>
            Georgia&apos;s premier quarterly business publication — distributed across a curated ecosystem of luxury touchpoints.
          </p>
        </div>
      </section>
        ...
        <section style={{ padding: '0 0 80px' }}>
          <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 48px' }}>
            <div style={{ marginBottom: '24px' }}>
              <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#fff', marginBottom: '8px' }}>Issue #{current.number} — {current.title}</h2>
              <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em' }}>{current.quarter} {current.year}</p>
            </div>
            <FlipbookViewer issue={current} />
          </div>
        </section>
      )}
      ...
      <section style={{ padding: '40px 0 120px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 48px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
            {issues.map((issue, i) => (
              <button key={issue._id ?? i} onClick={() => setSelected(i)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', padding: 0 }}>
                <div style={{ position: 'relative', aspectRatio: '3/4', background: '#111', marginBottom: '12px', overflow: 'hidden', outline: i === selected ? '2px solid #A38560' : '2px solid transparent', outlineOffset: '2px', transition: 'outline-color 0.2s' }}>
                  {issue.cover
                    ? <img src={issue.cover} alt={issue.title} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.7 }} />
